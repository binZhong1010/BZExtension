...
//    NSLog(@"removeAll");
    ...
    for (NSString* nameSpace in array)
    {
      ...
      if (![nameSpace isEqualToString:@"EYBaseItemDataCache"]) {
...
//    NSLog(@"initWithNameSpace");
    ...
    if (self) {
        ...
        if (configDictory && policy != IDPCacheStorageMemory) {
            ...
            NSNumber* diskExpiredTime = [configDictory numberAtPath:kIdpCacheConfigDiskExpiredTime otherwise:[NSNumber numberWithUnsignedInteger:365*24*60*60]];
...
//            self.diskCacheSize = 20 * 1024 * 1024;
            //一周
            self.diskExpiredTime = 365*24*60*60;
