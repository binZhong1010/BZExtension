...
- (NSError *)dbInsertIntoTable:(NSString *)table values:(NSArray *)values{
  ...
  NSError *error;
  [self executeUpdate:[NSString stringWithFormat:@"insert or ignore into %@ values(%@)",table,valueString] withErrorAndBindings:&error];
  
//  NSLog(@" insert error %@ -------- %@ ", error, values);
  return error;
}

- (NSError *)dbInsertIntoTable:(NSString *)table columns:(NSArray *)columns values:(NSArray *)values{
  NSString *valueString = @"";
  for (id obj in values) {
    if ([obj isKindOfClass:[NSString class]]) {
      valueString = [valueString stringByAppendingString:[NSString stringWithFormat:@"'%@'",obj]];
  ...
  for (id obj in columns) {
    if ([obj isKindOfClass:[NSString class]]) {
      columnString = [columnString stringByAppendingString:[NSString stringWithFormat:@"'%@'",obj]];
    }
    else if ([obj isKindOfClass:[NSNumber class]]){
      columnString = [columnString stringByAppendingString:[(NSNumber *)obj stringValue]];
    }
    columnString = [columnString stringByAppendingString:@","];
  }
  if (columnString.length>=1) {
    columnString = [columnString substringWithRange:NSMakeRange(0, columnString.length-1)];
  }
  
  
  NSError *error;
  [self executeUpdate:[NSString stringWithFormat:@"insert or ignore into %@ (%@) values(%@)",table, columnString,valueString]
 withErrorAndBindings:&error];
