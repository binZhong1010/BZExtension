...
/** Represents the results of executing a query on an `<ALDataBase>`.
...
@interface ALResultSet : NSObject {
    ALDataBase          *_parentDB;
    ALStatement         *_statement;
...
/** `ALStatement` used by result set. */

@property (atomic, retain) ALStatement *statement;

///------------------------------------
/// @name Creating and closing database
///------------------------------------

/** Create result set from `<ALStatement>`
 
 @ param statement A `<ALStatement>` to be performed
 
 @ param aDB A `<ALDataBase>` to be used
 
 @return A `ALResultSet` on success; `nil` on failure
 */

+ (instancetype)resultSetWithStatement:(ALStatement *)statement usingParentDatabase:(ALDataBase*)aDB;

/** Close result set */

- (void)close;

- (void)setParentDB:(ALDataBase *)newDb;
