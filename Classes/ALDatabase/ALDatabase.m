...
- (ALResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args;
...
#pragma mark ALDataBase instantiation and deallocation
...
// if ([ALDataBase FMDBVersion] >= 0x0230) { … }
...
static int FMDBDatabaseBusyHandler(void *f, int count) {
  ALDataBase *self = (__bridge ALDataBase*)f;
...
// but for folks who don't bother noticing that the interface to ALDataBase changed,
...
- (void)closeOpenResultSets {
  ...
  for (NSValue *rsInWrappedInATastyValueMeal in openSetCopy) {
    ALResultSet *rs = (ALResultSet *)[rsInWrappedInATastyValueMeal pointerValue];
...
- (void)resultSetDidClose:(ALResultSet *)resultSet {
...
- (ALStatement*)cachedStatementForQuery:(NSString*)query {
  
  NSMutableSet* statements = [_cachedStatements objectForKey:query];
  
  return [[statements objectsPassingTest:^BOOL(ALStatement* statement, BOOL *stop) {
...
- (void)setCachedStatement:(ALStatement*)statement forQuery:(NSString*)query {
...
- (BOOL)goodConnection {
  ...
  ALResultSet *rs = [self executeQuery:@"select name from sqlite_master where type='table'"];
...
- (void)warnInUse {
  NSLog(@"The ALDataBase %@ is currently in use.", self);
  
#ifndef NS_BLOCK_ASSERTIONS
  if (_crashOnErrors) {
    NSAssert(false, @"The ALDataBase %@ is currently in use.", self);
    abort();
  }
#endif
}
...
- (BOOL)databaseExists {
  ...
  if (!_db) {
    ...
    NSLog(@"The ALDataBase %@ is not open.", self);
    
#ifndef NS_BLOCK_ASSERTIONS
    if (_crashOnErrors) {
      NSAssert(false, @"The ALDataBase %@ is not open.", self);
      abort();
    }
#endif
...
- (NSError*)errorWithMessage:(NSString*)message {
  ...
  return [NSError errorWithDomain:@"ALDataBase" code:sqlite3_errcode(_db) userInfo:errorMessage];
}
...
- (ALResultSet *)executeQuery:(NSString *)sql withParameterDictionary:(NSDictionary *)arguments {
...
//  NSLog(@"execute sql:%@",sql);
  ...
  if (!statement) {
    statement = [[ALStatement alloc] init];
  ...
  rs = [ALResultSet resultSetWithStatement:statement usingParentDatabase:self];
  [rs setQuery:sql];
...
- (ALResultSet *)executeQuery:(NSString*)sql, ... {
  va_list args;
  va_start(args, sql);
...
- (ALResultSet *)executeQueryWithFormat:(NSString*)format, ... {
  va_list args;
...
- (ALResultSet *)executeQuery:(NSString *)sql withArgumentsInArray:(NSArray *)arguments {
  return [self executeQuery:sql withArgumentsInArray:arguments orDictionary:nil orVAList:nil];
}

- (ALResultSet *)executeQuery:(NSString*)sql withVAList:(va_list)args {
...
- (BOOL)executeUpdate:(NSString*)sql error:(NSError**)outErr withArgumentsInArray:(NSArray*)arrayArgs orDictionary:(NSDictionary *)dictionaryArgs orVAList:(va_list)args {
//  NSLog(@"execute update sql:%@",sql);
  if (![self databaseExists]) {
    NSLog(@"execute update error: database not exists");
    if (outErr) {
      *outErr = [self errorWithMessage:@"execute update error: database not exists"];
    }
    return NO;
  }
  ...
  if (_isExecutingStatement) {
    [self warnInUse];
    NSLog(@"execute update error: database in use");
    if (outErr) {
      *outErr = [self errorWithMessage:@"execute update error: database in use"];
    }
    return NO;
  }
  ...
  if (idx != queryCount) {
    ...
    _isExecutingStatement = NO;
    if (outErr) {
      *outErr = [self errorWithMessage:[NSString stringWithFormat:@"Error: the bind count (%d) is not correct for the # of variables in the query (%d) (%@) (executeUpdate)", idx, queryCount, sql]];
    }
    return NO;
  }
  ...
  if (_shouldCacheStatements && !cachedStmt) {
    cachedStmt = [[ALStatement alloc] init];
  ...
  if(rc!=SQLITE_DONE&&rc!=SQLITE_OK){
    if (outErr) {
      *outErr = [self errorWithMessage:[NSString stringWithFormat:@"execute sql:%@ error",sql]];
    }
  }
...
int ALDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names); // shhh clang.
int ALDBExecuteBulkSQLCallback(void *theBlockAsVoid, int columns, char **values, char **names) {
...
- (BOOL)executeStatements:(NSString *)sql withResultBlock:(FMDBExecuteStatementsCallbackBlock)block {
  ...
  rc = sqlite3_exec([self sqliteHandle], [sql UTF8String], block ? ALDBExecuteBulkSQLCallback : nil, (__bridge void *)(block), &errmsg);
...
- (BOOL)update:(NSString*)sql withErrorAndBindings:(NSError**)outErr, ... {
  ...
  BOOL result = [self executeUpdate:sql error:outErr withArgumentsInArray:nil orDictionary:nil orVAList:args];
  if (!result&&outErr) {
    *outErr = [NSError errorWithDomain:@"update db failed" code:0 userInfo:nil];
  }
  va_end(args);
  return result;
}
...
void ALDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv); // -Wmissing-prototypes
void ALDBBlockSQLiteCallBackFunction(sqlite3_context *context, int argc, sqlite3_value **argv) {
#if ! __has_feature(objc_arc)
...
#if ! __has_feature(objc_arc)
  sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (void*)b, &ALDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#else
  sqlite3_create_function([self sqliteHandle], [name UTF8String], count, SQLITE_UTF8, (__bridge void*)b, &ALDBBlockSQLiteCallBackFunction, 0x00, 0x00);
#endif
}
