...
@property (nonatomic) AGCancelTouchScrollView *scrollView;
...
@implementation AGEmojiKeyboardView{
  NSString *_emojiResourcePath;
}

- (NSDictionary *)emojis {
  if (!_emojis) {
    if (_emojiResourcePath.length) {
      _emojis = [[NSDictionary dictionaryWithContentsOfFile:_emojiResourcePath] copy];
    }else{
...
- (NSArray *)imagesForSelectedSegments {
  ...
  dispatch_once(&onceToken, ^{
    ...
    for (AGEmojiKeyboardViewCategoryImage i = AGEmojiKeyboardViewCategoryImageRecent;
         ...
         ++i) {
      UIImage *image = [self.dataSource emojiKeyboardView:self imageForSelectedCategory:i];
      if (image && [image isKindOfClass:[UIImage class]]) {
        [array safeAddObject:image];
      }else{
        [array safeAddObject:[UIImage imageWithColor:[UIColor clearColor] size:CGSizeMake(50, 50)]];
      }
    }
  });
  return array;
...
- (instancetype)initWithFrame:(CGRect)frame dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource{
  return [self initWithFrame:frame dataSource:dataSource emojiResourcePath:nil];
}

- (instancetype)initWithFrame:(CGRect)frame
                   dataSource:(id<AGEmojiKeyboardViewDataSource>)dataSource
            emojiResourcePath:(NSString *)emojiResourcePath{
  self = [super initWithFrame:frame];
  if (self) {
    // initialize category
    _emojiResourcePath = emojiResourcePath;
    ...
    [self.btnSend setTitle:NSLocalizedString(@"发送",nil) forState:UIControlStateNormal];
    ...
    self.scrollView = [[AGCancelTouchScrollView alloc] initWithFrame:CGRectMake(0,
                                                                                7.5,
    ...
    UIView *line = [[UIView alloc] initWithFrame:CGRectMake(0, 171, UIScreenWidth, 0.5)];
    line.backgroundColor = [UIColor colorWithRGBHex:0xe0e0e0];
...
- (void)layoutSubviews {
  CGSize pageControlSize = [self.pageControl sizeForNumberOfPages:3];
  NSUInteger numberOfPages = [self numberOfPagesForCategory:self.category
                                                inFrameSize:CGSizeMake(self.width,self.height - self.segmentsBar.height - pageControlSize.height)];
  ...
  self.scrollView.frame = CGRectMake(0,7.5,UIScreenWidth,134);
...
- (NSUInteger)numberOfRowsForFrameSize:(CGSize)frameSize {
  return 3;//(NSUInteger)floor(frameSize.height / ButtonHeight);
}
