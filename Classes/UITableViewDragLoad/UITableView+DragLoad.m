...
- (void)dragTableHeaderDidTriggerRefresh:(DragTableHeaderView_ot*)view
{
  if (self.dragFooterView.isLoading)
  {
    ...
    if ([self.dragLoadDelegate respondsToSelector:@selector(dragTableLoadMoreCanceled:)])
    {
      [self.dragLoadDelegate dragTableLoadMoreCanceled:self];
    }
  }
...
- (void)dragTableFooterDidTriggerLoadMore:(DragTableFooterView_ot *)view
{
  if (self.dragHeaderView.isLoading)
  {
    ...
    if ([self.dragLoadDelegate respondsToSelector:@selector(dragTableRefreshCanceled:)])
    {
      [self.dragLoadDelegate dragTableRefreshCanceled:self];
    }
  }
...
@dynamic dragLoadDelegate;

- (id<UITableViewDragLoadDelegate>)dragLoadDelegate
{
  return objc_getAssociatedObject(self, DRAG_DELEGATE_KEY);
}

- (void)setDragLoadDelegate:(id<UITableViewDragLoadDelegate>)dragLoadDelegate refreshDatePermanentKey:(NSString *)refreshDatePermanentKey
{
  objc_setAssociatedObject(self, DRAG_DELEGATE_KEY, dragLoadDelegate, OBJC_ASSOCIATION_ASSIGN);
  if (dragLoadDelegate)
  {
...
- (void)callDelegateDidTriggerRefresh
{
  if ([self.dragLoadDelegate respondsToSelector:@selector(dragTableDidTriggerRefresh:)])
  {
    [self.dragLoadDelegate dragTableDidTriggerRefresh:self];
  }
}

- (void)callDelegateDidTriggerLoadMode
{
  if ([self.dragLoadDelegate respondsToSelector:@selector(dragTableDidTriggerLoadMore:)])
  {
    [self.dragLoadDelegate dragTableDidTriggerLoadMore:self];
  }
}
